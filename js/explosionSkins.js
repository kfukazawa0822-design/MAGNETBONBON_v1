...
// ══════════════════════════════════════════════════════════════════
// 爆発SE（効果音）：外部音声ファイルを使わず、Web Audio APIで簡易合成した音を鳴らす。
// スキンごとに違う音を割り当てる。settings.seEnabled（設定画面のSEオン/オフ）に従う。
// ══════════════════════════════════════════════════════════════════
let __explosionAudioCtx = null;
function getExplosionAudioCtx(){
  if(!__explosionAudioCtx){
    try{ __explosionAudioCtx = new (window.AudioContext||window.webkitAudioContext)(); }
    catch(err){ return null; }
  }
  // ブラウザによっては最初のユーザー操作前はsuspended状態なので、そのつど再開を試みる
  if(__explosionAudioCtx.state === 'suspended') __explosionAudioCtx.resume().catch(()=>{});
  return __explosionAudioCtx;
}

// 【デフォルト】軽い「ポン」という音
function playExplosionSE_default(){
  const ac = getExplosionAudioCtx(); if(!ac) return;
...
// 【花火】ヒュー↑（打ち上げ音風）＋パンッ（ノイズの破裂音）の2段構成
function playExplosionSE_fireworks(){
  const ac = getExplosionAudioCtx(); if(!ac) return;
  const t0 = ac.currentTime;
  const o = ac.createOscillator(), g = ac.createGain();
  o.type = 'sine';
  o.frequency.setValueAtTime(600, t0);
  o.frequency.exponentialRampToValueAtTime(1500, t0+0.08);
  g.gain.setValueAtTime(0.001, t0);
  ...
  ng.gain.setValueAtTime(0.16, t0+0.06);
  ng.gain.exponentialRampToValueAtTime(0.001, t0+0.32);
  noise.connect(ng); ng.connect(ac.destination);
  noise.start(t0+0.06);
}

// 【ネオンドット破片】デジタルで鋭い「ピュン」というザップ音
function playExplosionSE_neonDebris(){
  const ac = getExplosionAudioCtx(); if(!ac) return;
  const t0 = ac.currentTime;
  const o = ac.createOscillator(), g = ac.createGain();
  o.type = 'sawtooth';
...
// 【漫画風トゲトゲ爆発】短く固い「ドンッ」という衝撃音（低音の減衰音＋一瞬のノイズ）
function playExplosionSE_mangaBurst(){
  const ac = getExplosionAudioCtx(); if(!ac) return;
  const t0 = ac.currentTime;
  const o = ac.createOscillator(), g = ac.createGain();
  o.type = 'square';
...
const EXPLOSION_SE = {
  default: playExplosionSE_default,
  fireworks: playExplosionSE_fireworks,
  neonDebris: playExplosionSE_neonDebris,
  mangaBurst: playExplosionSE_mangaBurst,
};

// index.html側のtriggerExplosion()から、爆発が起きるたびに1回呼び出す想定
function playExplosionSE(skinId){
  if(typeof settings !== 'undefined' && settings && settings.seEnabled === false) return;
  const fn = EXPLOSION_SE[skinId] || EXPLOSION_SE.default;
  try{ fn(); } catch(err){ /* Web Audio非対応環境などは無視 */ }
}
